...
export default function ProductsPage() {
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success(`Unit ${name} decommissioned.`, { id: toastId });
        dispatch(fetchProducts());
      }
    } catch (err) {
    ...
    <div className="flex flex-col space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 border-b border-white/5 pb-8">
        <div className="space-y-2">
           <h1 className="text-4xl font-head font-black text-white uppercase tracking-tighter leading-none">Product </h1>
           <p className="text-sm text-white/40 font-medium italic flex items-center gap-2 uppercase tracking-widest text-[10px]">
              <Target size={12} className="text-gold" /> Cataloging tactical assets and deployment status.
           </p>
        </div>
        <div className="flex items-center gap-4">
           <button 
                ...
                disabled={importing}
           >
                <Upload size={16} className="group-hover:-translate-y-0.5 transition-transform" /> Import Batch
           </button>
      ...
      <div className="flex flex-col lg:flex-row items-center gap-6 bg-charcoal p-5 rounded-sm border border-white/5 shadow-2xl shadow-black/60">
         <div className="relative flex-1 group w-full">
            ...
            <input 
               placeholder="IDENTIFY ASSET BY NAME OR SERIAL SKU..." 
               value={searchQuery}
         ...
         <div className="flex items-center gap-3 w-full lg:w-auto">
            ...
            <button className="h-12 px-6 border border-white/10 text-white/40 hover:text-white hover:border-gold/30 font-head font-bold text-xs uppercase tracking-widest rounded-sm transition-all flex items-center justify-center gap-2 italic">
               <Download size={16} /> Export Stream
            </button>
         </div>
      </div>
        ...
        <Table>
          <TableHeader className="bg-ink/60 border-b border-white/5">
            <TableRow className="hover:bg-transparent border-white/5 h-16">
              <TableHead className="text-[10px] font-black uppercase tracking-[0.25em] text-white/20 px-8">Identified Unit</TableHead>
              <TableHead className="text-[10px] font-black uppercase tracking-[0.25em] text-white/20">Serial Logic</TableHead>
              <TableHead className="text-[10px] font-black uppercase tracking-[0.25em] text-white/20">Operational Status</TableHead>
              <TableHead className="text-[10px] font-black uppercase tracking-[0.25em] text-white/20">Configuration</TableHead>
              <TableHead className="text-[10px] font-black uppercase tracking-[0.25em] text-white/20 text-right px-8">Asset Control</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {loading ? (
              <TableRow className="border-none hover:bg-transparent">
                <TableCell colSpan={5} className="h-64 text-center">
                   <div className="flex flex-col items-center gap-4">
                      ...
                      <span className="text-[10px] font-black uppercase tracking-[0.4em] text-white/20 italic animate-pulse">Syncing Inventory Nodes...</span>
                   </div>
                </TableCell>
              </TableRow>
                ...
                <TableCell colSpan={5} className="h-96 text-center p-12">
                   <div className="flex flex-col items-center gap-6 text-white/10 italic">
                      <div className="h-24 w-24 rounded-full border border-white/5 flex items-center justify-center bg-white/[0.02] shadow-inner">
                         <Package size={48} strokeWidth={1} className="opacity-40" />
                      </div>
                      <span className="text-sm font-bold uppercase tracking-[0.3em] leading-relaxed">No products identified in current sub-grid.</span>
                   </div>
                </TableCell>
              </TableRow>
            ) : (
              filteredProducts.map((prod) => (
                <TableRow key={prod._id} className="group border-white/5 hover:bg-white/[0.02] transition-all duration-300">
                     ...
                     <div className="flex items-center gap-6">
                        <div className="h-16 w-16 rounded-sm overflow-hidden bg-ink border border-white/5 group-hover:border-gold/30 transition-all ring-1 ring-gold/0 group-hover:ring-gold/5">
                           {prod.primaryImageId || (prod.gallery && prod.gallery[0]) ? (
                             <img src={prod.primaryImageId || prod.gallery[0]} alt={prod.name} className="h-full w-full object-cover grayscale-[0.3] group-hover:grayscale-0 transition-all duration-500" />
                           ) : (
                             <div className="h-full w-full flex items-center justify-center text-white/10"><ImageIcon size={22} /></div>
                           )}
                        </div>
                        <div className="flex flex-col space-y-1.5 overflow-hidden">
                           <span className="text-sm font-bold text-white uppercase tracking-tight leading-none group-hover:text-gold transition-colors italic truncate">{prod.name}</span>
                           <span className="text-lg font-head font-black text-gold/80 tracking-tighter leading-none">${prod.price}</span>
                        </div>
                     </div>
                  </TableCell>
                  <TableCell>
                     <span className="text-[9px] font-black text-white/20 px-3 py-1 bg-ink border border-white/5 rounded-sm uppercase tracking-widest italic group-hover:text-gold/40 group-hover:border-gold/20 transition-all">{prod.sku || 'UNASSIGNED'}</span>
                  </TableCell>
                  <TableCell>
                     <div className={cn(
                       ...
                       <div className={cn("h-1.5 w-1.5 rounded-full ring-2 ring-white/0 group-hover:ring-current/20", prod.status === 'active' ? "bg-emerald-400" : "bg-white/30")} />
                       {prod.status}
                     </div>
                  </TableCell>
                  <TableCell>
                     <div className="flex items-center gap-3 text-white/30 italic group-hover:text-gold/50 transition-colors">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-widest">{prod.variantCount || 0} OP-Variants</span>
                     </div>
                  </TableCell>
