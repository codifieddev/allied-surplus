...
>("categories/createCategory", async (payload, { rejectWithValue }) => {
  try {
    ...
    return data.data; // Use the inner data object
  } catch (error: any) {
    return rejectWithValue({
      message: error?.message || "Something went wrong",
    });
...
>("categories/deleteCategory", async (id, { rejectWithValue }) => {
  try {
    ...
    if (!res.ok) {
      return rejectWithValue({
        message: data?.message || data?.error || "Failed to delete category",
        status: res.status,
      });
    }
    return id;
...
>("categories/bulkImport", async (categories, { rejectWithValue }) => {
  try {
    ...
    if (!res.ok) {
      return rejectWithValue({
        message: data?.message || data?.error || "Failed to import categories",
        status: res.status,
      });
    }
    return data;
