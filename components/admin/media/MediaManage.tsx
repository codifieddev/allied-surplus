            ...
            {["upload", "library"].map((tab) => (
              <button
                ...
                className={`relative px-6 py-2 text-xs font-black uppercase tracking-widest transition-all ${
                  activeTab === tab ? "bg-gold text-ink" : "text-slate-400 hover:text-gold"
                }`}
              >
            ...
            >
              ...
              {uploadMethod === "file" ? (
                <div className="space-y-6">
                  ...
                  >
                    ...
                    <div className="w-20 h-20 mx-auto mb-6 bg-ink border border-charcoal-light flex items-center justify-center group-hover:border-gold transition-colors">
                      <Upload className="text-slate-500 group-hover:text-gold transition-colors" size={32} />
                    </div>
                  ...
                  {selectedFiles.length > 0 && (
                    <motion.div className="space-y-4">
                      {selectedFiles.map((file, index) => (
                        <div key={index} className="bg-charcoal border border-charcoal-light p-4 flex gap-6">
                          <img src={file.preview} alt="" className="w-24 h-24 object-cover border border-charcoal-light" />
                          <div className="flex-1 space-y-4">
                            <div className="flex justify-between items-start">
                              <div>
                                <p className="text-xs font-black text-gold uppercase tracking-widest truncate max-w-xs">{file.filename}</p>
                                <p className="text-[10px] font-bold text-slate-500">{file.size}</p>
                              </div>
                              <button onClick={() => removeFile(index)} className="p-2 hover:bg-red-500/10 text-slate-600 hover:text-red-500 transition-all">
                                <X size={18} />
                              </button>
                            </div>
                ...
                <div className="bg-charcoal border border-charcoal-light p-8">
                  <label className="block text-[10px] font-black text-gold uppercase tracking-widest mb-4">Remote Intel Link</label>
                  <div className="flex gap-4">
                    ...
                    />
                    <button onClick={handleUrlUpload} className="px-8 py-4 bg-gold text-ink font-black uppercase tracking-widest text-xs hover:bg-white transition-all flex items-center gap-2">
            ...
            >
                ...
                <div className="flex flex-wrap gap-2 pb-4 border-b border-charcoal-light">
                  <button
                    ...
                    className={`px-4 py-2 text-[10px] font-black uppercase tracking-widest transition-all ${
                      selectedFolder === null ? "bg-gold text-ink" : "bg-charcoal text-slate-400 border border-charcoal-light"
                    }`}
                  >
                    All Sectors
                  </button>
                  {folders.map((folder: any) => (
                    <button
                      ...
                      className={`px-4 py-2 text-[10px] font-black uppercase tracking-widest transition-all ${
                        selectedFolder === folder ? "bg-gold text-ink" : "bg-charcoal text-slate-400 border border-charcoal-light"
                      }`}
                    >
                      {folder}
                    </button>
                  ...
                  {filteredMedia.map((item: any, index: number) => (
                    ...
                    >
                      ...
                      <div className="p-3 bg-charcoal-dark">
                        <p className="text-[9px] font-black text-slate-200 uppercase tracking-widest truncate">{item.filename}</p>
                        <div className="flex items-center gap-2 mt-1 opacity-50">
                          <Clock size={10} className="text-gold" />
                          <span className="text-[8px] font-bold text-slate-400 uppercase">{(item.size / 1024).toFixed(0)} KB</span>
                        </div>
                      </div>
              ...
              {filteredMedia.length === 0 && (
                <div className="text-center py-20 bg-charcoal border border-charcoal-light">
                  ...
                  <p className="text-xs font-black text-slate-500 uppercase tracking-widest">No assets localized in current sector.</p>
                </div>
              )}
            </motion.div>
          )}
                ...
                <div className="bg-ink p-6">
                  <div className="flex items-center justify-between mb-6 border-b border-charcoal pb-4">
                    <div className="flex items-center gap-3">
                       ...
                       <h3 className="text-lg font-black text-white uppercase tracking-[0.2em]">Asset <span className="text-gold">Intelligence</span></h3>
                    </div>
                    <button onClick={() => setSelectedMedia(null)} className="p-2 text-slate-500 hover:text-gold transition-colors">
                      <X size={24} />
                    </button>
                  </div>
                    ...
                    <div className="space-y-6">
                      <div className="space-y-4">
                        <div className="border-l-2 border-gold pl-4">
                          <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Codename</p>
                          <p className="text-sm font-black text-white uppercase tracking-widest">{selectedMedia.filename}</p>
                        </div>
                        <div className="border-l-2 border-olive pl-4">
                          <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Sector Path</p>
                          <p className="text-sm font-black text-white uppercase tracking-widest">{selectedMedia.foldername || "ROOT SECURE"}</p>
                        </div>
