          ...
          {[
            ...
            <button
              ...
              className={`w-full p-4 border rounded-sm flex items-center justify-between transition-all group ${
                status === s.id
                  ? "bg-white/5 border-gold shadow-[0_0_15px_rgba(251,191,36,0.1)]"
                  : "bg-ink border-white/5 opacity-40 hover:opacity-100 hover:border-gold/20"
              }`}
            >
              <div className="flex items-center gap-3">
                <div
                  className={`h-2 w-2 rounded-full ${status === s.id ? `bg-${s.color}-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]` : "bg-white/20"}`}
                />
                <span
                  className={`text-[10px] font-black uppercase tracking-widest ${status === s.id ? "text-white" : "text-white/40"}`}
                >
                  {s.label}
                </span>
              </div>
            ...
            <label
              ...
              className={`flex items-center gap-3 p-3 rounded-sm border cursor-pointer transition-all ${
                categoryIds.includes(String(cat.id))
                  ...
                  : "bg-ink/40 border-white/5 hover:border-gold/10"
              }`}
            >
              <div className="relative">
                ...
                <div className="h-4 w-4 bg-ink border border-white/20 rounded-none peer-checked:bg-gold peer-checked:border-gold transition-all flex items-center justify-center">
                  <Check
                    size={10}
                    className="text-ink font-black scale-0 peer-checked:scale-100 transition-transform"
                  />
                </div>
              </div>
              <div className="flex flex-col">
                <span
                  className={`text-[10px] font-black uppercase tracking-widest ${categoryIds.includes(String(cat.id)) ? "text-white" : "text-white/30"}`}
                >
                  {cat.name || cat.title}
                </span>
                <span className="text-[7px] font-bold text-white/10 uppercase tracking-widest italic">
                  {cat.slug}
                </span>
              </div>
            </label>
          ...
          {relatedProductCandidates.map((p) => (
            ...
            >
              <input
                ...
                onChange={() => onToggleRelatedProduct(p.id)}
              />
              <div className="h-4 w-4 bg-ink border border-white/20 rounded-none peer-checked:bg-gold peer-checked:border-gold transition-all flex items-center justify-center">
                <Check size={10} className="text-ink font-black" />
              </div>
              <span
                className={`text-[10px] font-black uppercase tracking-widest ${relatedProductIds.includes(p._id) ? "text-white" : "text-white/30"}`}
              >
                {p.name}
              </span>
            </label>
        ...
        <div className="space-y-4">
          ...
          <div className="p-3 bg-gold/5 border border-gold/10 rounded-sm">
            <p className="text-[8px] text-white/30 uppercase leading-relaxed font-bold tracking-tighter">
              Linked forms enable data capture during deployment (Custom Product
              Build).
            </p>
          </div>
        </div>
