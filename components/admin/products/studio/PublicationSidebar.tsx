...
import { Check, ChevronRight, Globe, LayoutGrid, Monitor, ShieldAlert, Zap } from "lucide-react";
           ...
           {allCategories.map((cat) => (
             <label 
               ...
               className={`flex items-center gap-3 p-3 rounded-sm border cursor-pointer transition-all ${
                 categoryIds.includes(cat._id) ? "bg-white/5 border-gold/30" : "bg-ink/40 border-white/5 hover:border-gold/10"
               }`}
             >
                ...
                <div className="flex flex-col">
                   <span className={`text-[10px] font-black uppercase tracking-widest ${categoryIds.includes(cat._id) ? "text-white" : "text-white/30"}`}>{cat.name || cat.title}</span>
        ...
        <div className="max-h-[200px] overflow-y-auto pr-2 space-y-2 custom-scrollbar">
           {relatedProductCandidates.map((p) => (
             <label 
               key={p._id}
               className={`flex items-center gap-3 p-3 rounded-sm border cursor-pointer transition-all ${
                 relatedProductIds.includes(p._id) ? "bg-white/5 border-gold/30" : "bg-ink/40 border-white/5"
               }`}
             >
                <input 
                  type="checkbox" 
