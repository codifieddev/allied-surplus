...
import { ArrowLeft, Eye, Package, Save, ShieldCheck, Terminal } from "lucide-react";
...
export function TacticalProductStudio() {
  ...
  const { allCategories } = useAppSelector((state: RootState) => state.adminCategories);
  const { allattributes } = useAppSelector((state: RootState) => state.adminAttributes);
  const { allProducts, currentProduct: form, saving, loading: productLoading } = useAppSelector((state: RootState) => state.adminProducts);
  ...
  useEffect(() => {
     if (isEditing && editId && allProducts.length > 0) {
        const singleProduct = allProducts.find((item: any) => item._id === editId);
        if (singleProduct) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (saveProduct.fulfilled.match(action)) {
        toast.success(isEditing ? "INTEL UPDATED" : "UNIT DEPLOYED", { id: tId });
  ...
  const regenerateVariants = () => {
    ...
    const nextVariants: VariantRow[] = combos.map((combo, index) => {
      ...
      return {
        ...
        title: existing?.title || buildCombinationTitle(combo) || `Variant ${index + 1}`,
        optionValues: combo,
  ...
  const toggleAttributeSet = (id: string) => {
    ...
    const nextOptions = isRemoving
      ...
      : [
          ...form.options,
          ...(allattributes.find((s: any) => (s.key || s._id) === id)?.attributes || [])
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-3">
             ...
             <h1 className="text-3xl font-black text-white uppercase tracking-tighter">
               {isEditing ? "Update" : "Deploy"} <span className="text-gold">Intelligence</span>
             </h1>
          </div>
          <div className="flex items-center gap-2 text-[10px] font-bold text-white/40 uppercase tracking-widest italic">
             ...
             {isEditing ? `Modifying Unit ${form.sku}` : "Creating New Asset Node"}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 space-y-8">
          <GeneralInformation
            ...
            onFormChange={(field, value) => dispatch(setProductFormField({ field: field as any, value }))}
            onSlugChange={setProductSlug}
          />

          <PricingInventory
            pricing={form.pricing}
            onPricingChange={(field, value) => dispatch(setPricingField({ field: field as any, value }))}
          />
          ...
          <VisualMedia
            ...
            onGalleryChange={(gallery) => dispatch(setProductFormField({ field: "gallery", value: gallery }))}
            onPrimaryImageChange={(id) => dispatch(setProductFormField({ field: "primaryImageId", value: id }))}
            ...
            onAddGalleryItem={(item) => {
              ...
              if (!form.primaryImageId) {
                dispatch(setProductFormField({ field: "primaryImageId", value: item.id }));
              }
            }}
          />
          ...
          <VariantMatrix
            ...
            onVariantsChange={(variants) => dispatch(setProductFormField({ field: "variants", value: variants }))}
          />
        </div>
